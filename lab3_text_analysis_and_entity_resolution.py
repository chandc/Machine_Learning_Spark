...
def simpleTokenize(string):
    ...
    """
    s_string = re.split(split_regex,string.lower())
    
    return filter(None, s_string)
...
def tokenize(string):
    ...
    """
    s_token = simpleTokenize(string)
    s_token = [x for x in s_token if x not in stopwords]
...
def findBiggestRecord(vendorRDD):
    ...
    numberofrecords = vendorRDD.count()    
    topCounts = vendorRDD.takeOrdered(numberofrecords, key=lambda (k,x): -len(x))
    return topCounts
...
# N is the number of documents
    N = corpus.count()
    uniqueTokens = corpus.map(lambda (k,v): (k,set(v)))
#    print uniqueTokens.take(30)
    tokenCountPairTuple = (uniqueTokens
                          .flatMap(lambda (k,v): v)
                          .map(lambda v: (v,1)))
#    print tokenCountPairTuple.takeOrdered(10)
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(add)
#    print tokenSumPairTuple.takeOrdered(50,key=lambda (k,v): +(v))
    return (tokenSumPairTuple.map(lambda (k,v) : (k,float(N)/float(v))))
...
#    print tokenize(string1)
#    print tokenize(string2)
#    print idfsDictionary
...
# TODO: Replace <FILL IN> with appropriate code
from operator import add
sims = similaritiesBroadcast.map(lambda (k1,k2,v) : 
               ('%s %s' % (removeQuotes(k2), removeQuotes(k1)),v))

trueDupsRDD = (sims
               .join(goldStandard))
trueDupsCount = trueDupsRDD.count()
avgSimDups = (trueDupsRDD.map(lambda (k,(cos,gold)): cos/float(trueDupsCount))
              .reduce(add)  )
...
avgSimNon = (nonDupsRDD.map(lambda (k,cos): cos/float(nonDupsCount))
              .reduce(add) )
#print (sims
#       .map(lambda (k1,k2,v) : 
#        ('%s %s' % (removeQuotes(k2), removeQuotes(k1)),v)).take(10))
#print trueDupsRDD.take(2)
...
#print amazonWeightsRDD.top(5)
#print googleWeightsRDD.top(5)
...
#print amazonWeightsRDD.map(lambda v: v[1]).top(1)
amazonNorms = amazonWeightsRDD.map(lambda v: (v[0],norm(v[1]))).collectAsMap()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.map(lambda v: (v[0],norm(v[1]))).collectAsMap()
...
def invert(record):
    ...
    for rc in record:
            for token in rc[1]:
                pairs.append((token, rc[0]))
    return (pairs)

tt = [(1, {'token a': 2.4051362683438153, 'token b': 56.65432098765432, 'token c': 254.94444444444443})]
print invert(tt)
#t5 = [('b000jz4hqo', {'rom': 2.4051362683438153, 'clickart': 56.65432098765432, '950': 254.94444444444443, 'image': 3.6948470209339774, 'premier': 9.27070707070707, '000': 6.218157181571815, 'dvd': 1.287598204264871, 'broderbund': 22.169082125603865, 'pack': 2.98180636777128})]
