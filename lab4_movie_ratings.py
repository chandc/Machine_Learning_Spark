...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    avg = sum(IDandRatingsTuple[1])/float(N)
    return (MovieID,(N,avg))
...
#print movieNameWithAvgRatingsRDD.collect()
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda (avgRating,movieName,numRating) : numRating > 500 )
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                       ...
                       .map(lambda ((a,b),(c,d)): float((c-d)*(c-d))   )
                       )

#    print squaredErrorsRDD.top(10)
...
#    print numRatings
    ...
    rmse = math.sqrt((totalError/numRatings)) 
...
trainingAvgRating = trainingRDD.map(lambda (a,b,c) : c).reduce(add)/trainingRDD.count()
...
myUserID = 0
myrating = 3.0
gg = movieIDsWithAvgRatingsRDD.map(lambda (a,(b,c)): (myUserID,b,myrating))
...
myRatedMovies = [
     (0, 1775, 3.0), (0, 1447, 3.0), (0, 1438, 3.0), (0, 1423, 3.0), 
    (0, 1390, 3.0), (0, 1370, 3.0), (0, 1337, 3.0), (0, 1320, 3.0), 
    (0, 1300, 3.0), (0, 1250, 3.0), (0, 1248, 3.0), (0, 1218, 3.0), 
    (0, 1203, 3.0), (0, 1195, 3.0), (0, 1184, 3.0)
...
#print predictedTestMyRatingsRDD.take(5)
...
s=[]
while counter <= n-1:
    s.append(myRatedMovies[counter][1])
    counter = counter + 1

myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda (a,b): a not in s) 
                      .map(lambda (a,b) : (myID,a))
                     )

print myUnratedMoviesRDD.top(10)
...
#print movieCountsRDD.take(5)

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda (uid,mid,rating) : (mid,rating))

#print predictedRDD.take(5)
...
#print predictedWithCountsRDD.take(5)
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)
                       .map(lambda (ID,((pr,nRate),title)) : (pr,title,nRate))
                      )
