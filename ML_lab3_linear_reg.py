...
def calcRMSE(labelsAndPreds):
    ...
    """
    ss = labelsAndPreds.map(lambda x: squaredError(x[0],x[1])).mean()
...
def getLabeledPrediction(weights, observation):
    ...
    values = DenseVector(observation.features)
    prediction = weights.dot(values)
...
labelsAndPreds = parsedValData.map(lambda lp: (lp.label,weightsLR0.dot(DenseVector(lp.features))))
...
def twoWayInteractions(lp):
    ...
    """
    key = lp.label
    values = DenseVector(lp.features)
    interactions =itertools.product(values,values)
    s = []
    for elem in interactions:
        s.append(elem[0]*elem[1])
    
    s = DenseVector(s)
    
    new_lp_features = np.hstack((values,s))
    return LabeledPoint(key,new_lp_features)


#LabeledPoint(s[0], s[1:])
